...
parser.add_argument('--ExpRslt','-er',type=bool,help='Flag to indicate if export results [default:False]',default=True)    # bool
...
parser.add_argument('--Rampup','-rp',type=int,help='Number of epochs to rampup the weakly supervised losses [default: 51]',default=1)
...
parser.add_argument('--Style','-sty',type=str,help='Style for training the network [default: Full]'
                                                     ' [options: Plain, Full, weight]',default='Plain')
...
if args.ExpRslt:
    ...
    print("Files will saved in", CHECKPOINT_PATH)
    ...
    fid.close()
else: raise ValueError("File will not be saved.")
##### Define Network
if args.Style == 'Full' or args.Style == 'weight':
